#!/usr/bin/env python3
"""
Stage 01 - Parse: Process emails through joke-extract.py and prepare for pipeline.
"""
...
class ParseProcessor(StageProcessor):
  ...
  def __init__(self):
    """Initialize the Parse processor."""
    super().__init__(
      stage_name="parse",
      input_stage=config.STAGES["parse"],
      output_stage=config.STAGES["dedup"],
      reject_stage=config.REJECTS["parse"],
      config_module=config
    )
    self.logger = get_logger("ParseProcessor")

#!/usr/bin/env python3
"""
Tests for stage_title.py - Title generation and final validation.
"""
...
def setup_test_environment():
  ...
  test_dir = tempfile.mkdtemp(prefix="test_title_")
  ...
  os.makedirs(os.path.join(pipeline_main, "55_rejected_title"))
  ...
  yield {
    ...
    'input_dir': os.path.join(pipeline_main, "06_title"),
    'output_dir': os.path.join(pipeline_main, "08_ready_for_review"),
    'reject_dir': os.path.join(pipeline_main, "55_rejected_title")
  }
...
def mock_ollama_title_generation():
  ...
  with patch('stage_title.OllamaClient') as mock_client_class:
    mock_client = Mock()
...
def test_existing_title_preserved(setup_test_environment):
  ...
  assert headers['Title-Source'] == 'Submitter'
...
def test_blank_title_generates(
  ...
  assert headers['Title-Source'] == 'LLM'
...
def test_complete_flow_with_title_generation(
  ...
  assert 'Title-Reason' in headers
  assert 'Title-LLM-Model-Used' in headers

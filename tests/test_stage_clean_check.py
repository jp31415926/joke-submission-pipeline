#!/usr/bin/env python3
"""
Tests for stage_clean_check.py - Cleanliness check using LLM.
"""
...
def setup_test_environment():
  ...
  test_dir = tempfile.mkdtemp(prefix="test_clean_check_")
  pipeline_main = os.path.join(test_dir, "pipeline-main")
  pipeline_priority = os.path.join(test_dir, "pipeline-priority")

  # Create directory structure
  os.makedirs(os.path.join(pipeline_main, "03_clean_check"))
  os.makedirs(os.path.join(pipeline_main, "04_format"))
  os.makedirs(os.path.join(pipeline_main, "52_rejected_clean_check"))
  ...
  yield {
    ...
    'input_dir': os.path.join(pipeline_main, "03_clean_check"),
    'output_dir': os.path.join(pipeline_main, "04_format"),
    'reject_dir': os.path.join(pipeline_main, "52_rejected_clean_check")
  }
...
def mock_ollama_pass_high_confidence():
  ...
  with patch('stage_clean_check.OllamaClient') as mock_client_class:
    mock_client = Mock()
...
def test_clean_joke_passes(
  ...
  assert headers['Pipeline-Stage'] == config.STAGES['format']
...
def test_questionable_joke_fails(
  ...
  assert headers['Pipeline-Stage'] == config.REJECTS['clean_check']
...
def test_metadata_updates(
  ...
  assert 'Clean-Check-Reason' in headers
  assert 'Clean-Check-LLM-Model-Used' in headers

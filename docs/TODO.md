...
1. [ ] From this point forward, when Claude makes changes, it should run `git commit` with a descriptive commit message. Update CLAUDE.md accordingly.
2. [ ] Sometimes `joke-pipeline.py --status` returns an error because it can't access a file (probably because it was moved). Make the script handle this properly.

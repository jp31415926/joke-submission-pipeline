...
class FormatProcessor(StageProcessor):
  """
  Process jokes to improve formatting using Ollama LLM.
  """

  def __init__(self):
    """Initialize the FormatProcessor."""
    super().__init__(
      stage_name="format",
      input_stage=config.STAGES["format"],
      output_stage=config.STAGES["categorize"],
      reject_stage=config.REJECTS["format"],
      config_module=config
    )
    self.logger = get_logger("FormatProcessor")
